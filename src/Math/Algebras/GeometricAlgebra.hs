module Math.Algebras.GeometricAlgebra
  ( mv
  , longMv
  , outerProd
  , projGrade
...
str2Bld str = foldl' bladeProd (0, 1) lst
  where
    lst = map basisVec2Bld $ bldRead str
...
bladeListSimp' (x:y:xs) =
  if bladeComp x y == EQ
    then bladeListSimp' ((bldAdd x y) : xs)
...
outerProd (Mv xs) (Mv ys) = Mv (bladeListSimp (sortBy bladeComp res))
  where
    res = [bladeProd x y | x <- xs, y <- ys, bladeGrade' (bladeProd x y) == (bladeGrade' x) + (bladeGrade' y)]
...
projGrade (Mv xs) m = Mv (filter isGrade xs)
  where
    isGrade (fst, _) = bladeGrade fst == m
